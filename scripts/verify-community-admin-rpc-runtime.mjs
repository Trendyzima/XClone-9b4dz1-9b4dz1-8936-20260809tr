...
const required = [
  ...
  'COMMUNITY_ADMIN_TEST_TARGET_USER_ID',
  'COMMUNITY_ADMIN_TEST_FORENSIC_KEY',
];
...
const targetUserId = process.env.COMMUNITY_ADMIN_TEST_TARGET_USER_ID;
...
try {
  ...
  const denied = await member.c.rpc('add_community_members', { p_community_id: communityId, p_user_ids: [targetUserId] });
  assert(denied.error, 'REGULAR_MEMBER_ADD_RPC_ALLOWED', { data: denied.data });
  assert((await membership(targetUserId)).length === 0, 'UNAUTHORIZED_RPC_CREATED_MEMBERSHIP');

  const added = await admin.c.rpc('add_community_members', { p_community_id: communityId, p_user_ids: [targetUserId] });
  if (added.error) throw new Error(`ADMIN_ADD_MEMBER_FAILED:${added.error.message}`);
  let rows = await membership(targetUserId);
  assert(rows.length === 1 && rows[0].status === 'active' && rows[0].role === 'member', 'ADMIN_MEMBER_STATE_INVALID', { rows });

  const duplicate = await admin.c.rpc('add_community_members', { p_community_id: communityId, p_user_ids: [targetUserId] });
  if (duplicate.error) throw new Error(`DUPLICATE_ADD_FAILED:${duplicate.error.message}`);
  rows = await membership(targetUserId);
  assert(rows.length === 1, 'DUPLICATE_MEMBERSHIP_CREATED', { rows });

  console.log(JSON.stringify({ ok: true, checks: ['two authenticated accounts','admin role boundary','regular member RPC denied','admin RPC allowed','membership persisted','duplicate add safe','forensic membership row verified'], communityId, targetUserId }, null, 2));
} catch (error) {
