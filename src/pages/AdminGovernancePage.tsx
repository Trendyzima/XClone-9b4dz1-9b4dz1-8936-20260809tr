...
import { useGovernance, listAdministrators, searchGovernanceUsers, listGovernanceAudit, listGovernancePermissions, inviteStaff, terminateStaff, updateAdministrator, listJobApplications, reviewJobApplication, GOVERNANCE_ROLES } from '@/lib/governance';
...
export default function AdminGovernancePage() {
  ...
  const load = async () => {
    setLoadingData(true);
    try { const [a, h, p, apps] = await Promise.all([listAdministrators(), listGovernanceAudit(), governance.is_owner ? listGovernancePermissions() : Promise.resolve([]), governance.is_owner ? listJobApplications() : Promise.resolve([])]); setAdmins(a); setAudit(h); setPermissions(p); setApplications(apps); }
  ...
  const appoint = async (userId: string, username: string) => {
    setBusy(userId);
    try { await inviteStaff(userId, role); toast.success('@' + username + ' invited — role features are active'); setResults([]); setQuery(''); setSelectedPermissions([]); await load(); }
