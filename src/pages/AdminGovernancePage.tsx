...
import { useGovernance, listAdministrators, searchGovernanceUsers, listGovernanceAudit, appointAdministrator, updateAdministrator, GOVERNANCE_ROLES } from '@/lib/governance';
...
export default function AdminGovernancePage() {
  ...
  const load = async () => {
    setLoadingData(true);
    try { const [a, h] = await Promise.all([listAdministrators(), listGovernanceAudit()]); setAdmins(a); setAudit(h); }
  ...
  const appoint = async (userId: string, username: string) => {
    setBusy(userId);
    try { await appointAdministrator(userId, role); toast.success('@' + username + ' appointed'); setResults([]); setQuery(''); await load(); }
  ...
  return <div className="min-h-screen bg-background pb-20">
    ...
    <div className="max-w-3xl mx-auto p-4 space-y-5">
      ...
      {governance.is_owner && <section className="rounded-2xl border border-border bg-card p-5 space-y-4"><div><h2 className="font-bold flex items-center gap-2"><UserPlus className="w-4 h-4" /> Appoint an administrator</h2><p className="text-xs text-muted-foreground mt-1">Search a Testagram profile, choose a least-privilege role, then appoint it.</p></div><div className="flex gap-2"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><input value={query} onChange={e => void search(e.target.value)} placeholder="Search username or display name" className="w-full h-10 rounded-xl border border-border bg-background pl-9 pr-3 text-sm" /></div><select value={role} onChange={e => setRole(e.target.value)} className="h-10 rounded-xl border border-border bg-background px-3 text-sm">{GOVERNANCE_ROLES.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}</select></div>{results.length > 0 && <div className="divide-y divide-border rounded-xl border border-border overflow-hidden">{results.map(user => <button key={user.user_id} onClick={() => void appoint(user.user_id, user.username)} disabled={!!busy} className="w-full flex items-center gap-3 p-3 bg-background hover:bg-muted text-left">{user.avatar_url ? <img src={user.avatar_url} alt="" className="w-9 h-9 rounded-full object-cover" /> : <div className="w-9 h-9 rounded-full bg-muted flex items-center justify-center font-bold">{user.username[0]?.toUpperCase()}</div>}<span className="flex-1"><b className="text-sm">@{user.username}</b><span className="block text-xs text-muted-foreground">{user.display_name ?? ''}</span></span>{busy === user.user_id ? <Loader2 className="w-4 h-4 animate-spin" /> : <UserPlus className="w-4 h-4 text-primary" />}</button>)}</div>}</section>}
