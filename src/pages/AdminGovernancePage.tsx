...
export default function AdminGovernancePage() {
  ...
  const [applications, setApplications] = useState<Awaited<ReturnType<typeof listJobApplications>>>([]);
  ...
  const load = async () => {
    setLoadingData(true);
    try { const [a, h, apps] = await Promise.all([listAdministrators(), listGovernanceAudit(), governance.is_owner ? listJobApplications() : Promise.resolve([])]); setAdmins(a); setAudit(h); setApplications(apps); }
  ...
  const appoint = async (userId: string, username: string) => {
    setBusy(userId);
    try { await inviteStaff(userId, role); toast.success('@' + username + ' invited — role features are active'); setResults([]); setQuery(''); await load(); }
  ...
  return <div className="min-h-screen bg-background pb-20">
    ...
    <div className="max-w-3xl mx-auto p-4 space-y-5">
      ...
      {governance.is_owner && <section className="rounded-2xl border border-border bg-card p-4 flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center"><Briefcase className="w-5 h-5"/></div><div className="flex-1 min-w-0"><p className="font-bold text-sm">Staff recruitment</p><p className="text-xs text-muted-foreground mt-1">{applications.length} application{applications.length === 1 ? '' : 's'} awaiting owner review and controlled invitation.</p></div><button onClick={() => window.location.assign('/admin/staff-recruitment')} className="inline-flex items-center gap-1.5 rounded-xl border border-border px-3 py-2 text-xs font-bold hover:bg-muted">Open <ArrowRight className="w-3.5 h-3.5"/></button></section>}
