...
export default function ThreadDetailPage(){
 ...
 const [loading,setLoading]=useState(true); const [replyTarget,setReplyTarget]=useState<string|null>(null); const [replyText,setReplyText]=useState(''); const [replyFiles,setReplyFiles]=useState<File[]>([]); const [replyPreviews,setReplyPreviews]=useState<string[]>([]); const [sending,setSending]=useState(false);
 ...
 const load=useCallback(async()=>{
  ...
  try{
   ...
   const qids=[...new Set((qrows??[]).map((q:any)=>q.user_id))]; const {data:qprofiles}=qids.length?await supabase.from('profiles').select('id,username,avatar_url,verified,display_name').in('id',qids):{data:[]}; const qpm=new Map((qprofiles??[]).map((p:any)=>[p.id,p as Profile]));
 ...
 const quote=async(threadId:string)=>{if(!requireAuth())return;const content=window.prompt('Add a thought to your quote');if(content===null)return;try{const {error}=await supabase.from('thread_quotes').insert({thread_id:threadId,user_id:user!.id,content:content.trim(),media_urls:[]});if(error)throw error;toast.success('Quote posted');await load();}catch(e:any){toast.error(e?.message||'Quote failed');}};
 ...
 return <div className="min-h-screen bg-background pb-20 md:pb-0"><TopBar title="Thread" showBack/><main>
  ...
  {replyTarget&&<div className="sticky bottom-0 z-20 border-t border-border bg-background/95 p-4 backdrop-blur-xl"><div className="mx-auto max-w-3xl">
   <div className="mb-2 text-xs font-semibold text-muted-foreground">Replying in the thread</div>
   <Textarea autoFocus value={replyText} onChange={e=>setReplyText(e.target.value.slice(0,500))} placeholder="Continue the conversation…" className="min-h-24 resize-none"/>
