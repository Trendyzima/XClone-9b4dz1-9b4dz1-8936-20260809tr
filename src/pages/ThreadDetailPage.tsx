...
import { InlineRssSuggestion } from '@/components/features/InlineRssSuggestion';
...
export default function ThreadDetailPage(){
 ...
 return <div className="min-h-screen bg-background pb-20 md:pb-0"><TopBar title="Thread" showBack/><main>
  <article className="px-4 py-5"><div className="flex gap-3"><button onClick={()=>navigate('/profile/'+(thread.profiles?.username||''))}><Avatar profile={thread.profiles}/></button><div className="min-w-0 flex-1">
   ...
   <p className="mt-4 whitespace-pre-wrap break-words text-[16px] leading-7">{thread.body}</p><InlineTvSuggestion content={thread.body} seed={thread.id} type="thread"/><InlineRssSuggestion content={thread.body} seed={thread.id+"-rss"} type="thread"/><Media items={thread.media_urls}/>
