...
export default function TvChannelsPage(){
 ...
 return <div className='min-h-screen bg-background'>
  <header className='sticky top-0 z-30 border-b bg-background/95 backdrop-blur p-3'><div className='max-w-3xl mx-auto flex items-center justify-between gap-3'><div><div className='flex items-center gap-2'><Radio className='w-5 h-5 text-red-500'/><h1 className='font-bold text-xl'>Testagram TV</h1></div><p className='text-xs text-muted-foreground'>Live channels · vertical viewing · no stream files stored by Testagram</p></div><div className='flex gap-2'><Button size='icon' variant='outline' onClick={()=>nav('/tv-studio')} title='Open TV Studio'><Clapperboard/></Button><Button size='icon' variant='outline' onClick={()=>{loaded.current.clear();setChannels([]);setSourceIndex(0);void loadSources(TV_SOURCES.slice(0,3).map(s=>s.id));}} title='Refresh'><RefreshCw/></Button></div></div>
  ...
  <main className='max-w-3xl mx-auto px-2 sm:px-3 py-3'>
   ...
   {!loading&&!filtered.length?<div className='py-20 text-center text-muted-foreground'><Globe2 className='mx-auto w-10 h-10 mb-3'/><p className='font-semibold'>No channels matched</p><p className='text-sm mt-1'>Try another country, category or source.</p></div>:
    <div className='snap-y snap-mandatory space-y-3'>{filtered.map(channel=><TvChannelPlayer key={channel.id} channel={channel} active={active===channel.id} onVisible={visible}/>)}
