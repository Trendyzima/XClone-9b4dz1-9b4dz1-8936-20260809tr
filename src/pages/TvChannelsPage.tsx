...
export default function TvChannelsPage(){
 ...
 return <div className={reelsMode?'h-[100svh] overflow-hidden bg-black':'min-h-screen bg-background'}>
  <header className='sticky top-0 z-30 border-b bg-background/95 backdrop-blur p-3'><div className='max-w-3xl mx-auto flex items-center justify-between gap-3'><div><div className='flex items-center gap-2'><Radio className='w-5 h-5 text-red-500'/><h1 className='font-bold text-xl'>{reelsMode?'TV Reels':'Testagram TV'}</h1></div><p className='text-xs text-muted-foreground'>{reelsMode?'Swipe vertically through live channels · one stream at a time':'Live channels · vertical viewing · no stream files stored by Testagram'}</p></div><div className='flex gap-2'><Button size='icon' variant='outline' onClick={()=>nav('/tv-studio')} title='Open TV Studio'><Clapperboard/></Button><Button size='icon' variant='outline' onClick={()=>{loaded.current.clear();setDead(new Set());setActive('');setChannels([]);setSourceIndex(0);void loadSources(TV_SOURCES.slice(0,3).map(s=>s.id));}} title='Refresh'><RefreshCw/></Button></div></div>
