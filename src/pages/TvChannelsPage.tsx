...
export default function TvChannelsPage(){
 const nav=useNavigate(); const {pathname}=useLocation(); const reelsMode=pathname==='/tv/reels'; const [channels,setChannels]=useState<TvChannel[]>([]); const [active,setActive]=useState(''); const [loading,setLoading]=useState(true); const [sourceIndex,setSourceIndex]=useState(0); const [filter,setFilter]=useState(''); const [query,setQuery]=useState(''); const [notice,setNotice]=useState(''); const loaded=useRef(new Set<string>());
 ...
 const filtered=useMemo(()=>{const q=query.trim().toLowerCase();return channels.filter(c=>{const text=(c.name+' '+(c.group||'')+' '+(c.language||'')).toLowerCase();const country=filter==='AF'?['KE','ZA','NG','GH','UG','TZ','RW','ZM','ZW','BW'].includes(c.country||''):filter?c.country===filter||text.includes(filter.toLowerCase()):true;return country&&(!q||text.includes(q));});},[channels,filter,query]);
 ...
 return <div className={reelsMode?'h-[100svh] overflow-hidden bg-black':'min-h-screen bg-background'}>
  <header className='sticky top-0 z-30 border-b bg-background/95 backdrop-blur p-3'><div className='max-w-3xl mx-auto flex items-center justify-between gap-3'><div><div className='flex items-center gap-2'><Radio className='w-5 h-5 text-red-500'/><h1 className='font-bold text-xl'>{reelsMode?'TV Reels':'Testagram TV'}</h1></div><p className='text-xs text-muted-foreground'>{reelsMode?'Swipe vertically through live channels · one stream at a time':'Live channels · vertical viewing · no stream files stored by Testagram'}</p></div><div className='flex gap-2'><Button size='icon' variant='outline' onClick={()=>nav('/tv-studio')} title='Open TV Studio'><Clapperboard/></Button><Button size='icon' variant='outline' onClick={()=>{loaded.current.clear();setChannels([]);setSourceIndex(0);void loadSources(TV_SOURCES.slice(0,3).map(s=>s.id));}} title='Refresh'><RefreshCw/></Button></div></div>
  ...
  <main className='max-w-3xl mx-auto px-2 sm:px-3 py-3'>
   ...
   {!loading&&!filtered.length?<div className='py-20 text-center text-muted-foreground'><Globe2 className='mx-auto w-10 h-10 mb-3'/><p className='font-semibold'>No channels matched</p><p className='text-sm mt-1'>Try another country, category or source.</p></div>:
    <div className={'snap-y snap-mandatory '+(reelsMode?'space-y-0':'space-y-3')}>{filtered.map(channel=><TvChannelPlayer key={channel.id} channel={channel} active={active===channel.id} onVisible={visible}/>)}
      {filtered.length>0&&<div className='snap-start py-10 text-center'><Button onClick={()=>void loadMore()} disabled={loading}><ChevronRight className='mr-2'/>Load more channels</Button><p className='text-xs text-muted-foreground mt-2'>{sourceLabel||'More public channel sources'} are loaded only as needed.</p></div>}
    </div>}
