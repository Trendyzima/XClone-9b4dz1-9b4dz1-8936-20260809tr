...
export default function HomeHubPage(){
  ...
  const fetchTab=useCallback(async(target:Tab,pageNum=0):Promise<Item[]>=>{
    ...
    const posts=(data??[]).map((x:any)=>({type:'post' as const,data:x}));
    if(target==='all'&&pageNum===0){const {data:threads}=await supabase.from('threads').select('*').eq('visibility','public').is('deleted_at',null).order('created_at',{ascending:false}).limit(4);return [...posts,...(threads??[]).map((x:any)=>({type:'thread' as const,data:x}))].sort((a,b)=>new Date(b.data.created_at).getTime()-new Date(a.data.created_at).getTime());}
    return posts;
  },[user?.id]);
