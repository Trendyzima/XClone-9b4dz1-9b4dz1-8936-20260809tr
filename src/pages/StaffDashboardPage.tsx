...
import { Activity, ArrowRight, CheckCircle2, ClipboardList, MessageSquare, ShieldCheck, Users, Wallet, Radio, Globe, FileText, LockKeyhole } from 'lucide-react';
...
export default function StaffDashboardPage(){
 ...
 return <div className="min-h-screen bg-background pb-20"><TopBar title="Staff Workspace" showBack/><div className="max-w-4xl mx-auto p-4 space-y-5">
  <section className="rounded-2xl border border-primary/20 bg-primary/5 p-5"><div className="flex gap-4 items-center"><div className="w-14 h-14 rounded-2xl bg-primary/10 text-primary flex items-center justify-center"><ShieldCheck className="w-7 h-7"/></div><div><p className="text-xs uppercase tracking-wide text-muted-foreground">Testagram staff</p><h1 className="font-black text-2xl">{role?.label??governance.role}</h1><p className="text-sm text-muted-foreground">{role?.description??'Assigned governance role'} · {governance.status}</p></div></div></section>
  <section className="grid sm:grid-cols-3 gap-3">{[['Role status','Active assignment',CheckCircle2],['Capabilities',String(caps.length),ShieldCheck],['Access model','Least privilege',LockKeyhole]].map(([a,b,I]:any)=><div key={a} className="rounded-2xl border border-border bg-card p-4"><I className="w-5 h-5 text-primary"/><p className="text-xs text-muted-foreground mt-3">{a}</p><p className="font-black mt-1">{b}</p></div>)}</section>
  <section className="rounded-2xl border border-border bg-card overflow-hidden"><div className="p-4 border-b border-border"><h2 className="font-bold">Your role command center</h2><p className="text-xs text-muted-foreground mt-1">Only capabilities granted to this assignment are shown.</p></div><div className="grid md:grid-cols-2">{caps.map(({key,label,icon:Icon,description})=><div key={key} className="p-4 border-b border-border md:[&:nth-child(odd)]:border-r"><div className="flex gap-3"><div className="w-9 h-9 rounded-xl bg-primary/10 text-primary flex items-center justify-center"><Icon className="w-4 h-4"/></div><div className="flex-1"><p className="font-bold text-sm">{label}</p><p className="text-xs text-muted-foreground mt-1">{description}</p><span className="inline-flex mt-2 text-[10px] px-2 py-1 rounded-full bg-primary/10 text-primary font-bold">AUTHORIZED</span></div></div></div>)}</div></section>
