...
export class TestagramCapabilityClient{
 ...
 async searchUnified(q:string,kind:"all"|"people"|"posts"|"hashtags"|"threads"|"communities"|"fediverse"|"media"|"latest"|"top"="all",n=40){const input={q,kind,limit:limit(n)};try{return await this.call<any>("testagram.search.unified",input)}catch(error){if(error instanceof CapabilityClientError&&error.code!=="AUTH_REQUIRED"){const token=await this.token();if(!token)throw error;try{const response=await fetch(`${supabaseUrl.replace(/\\/$/,"")}/rest/v1/rpc/testagram_search_unified`,{method:"POST",headers:{apikey:supabasePublishableKey,Authorization:`Bearer ${token}`,"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({p_q:q,p_kind:kind,p_limit:limit(n)})});const raw=await response.text();let payload:any=null;try{payload=raw?JSON.parse(raw):null}catch{}if(response.ok)return payload;const message=payload&&typeof payload==="object"&&"message" in payload?String(payload.message):`Search request failed (${response.status})`;throw new CapabilityClientError(message,{code:response.status===401||response.status===403?"AUTH_REQUIRED":"SEARCH_RPC_FAILED",status:response.status});}catch(fallbackError){if(fallbackError instanceof CapabilityClientError)throw fallbackError;}}throw error}}
