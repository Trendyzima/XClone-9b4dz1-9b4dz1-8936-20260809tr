...
// Canonical communication boundary. Testagram owns identity/authorization;
// LiveKit is the realtime media provider; Matrix/Novu remain downstream transports.
...
export const communicationService = {
  listConversations(limit = 50) {
    return backendCapabilities.call<{ items: CommunicationConversation[] }>('testagram.conversations.list', { limit });
  },
  createConversation(memberIds: string[]) {
    return backendCapabilities.call<{ conversation_id: string }>('testagram.conversations.create', { member_ids: memberIds });
  },
  listMessages(conversationId: string, limit = 50, cursor?: string) {
    return backendCapabilities.call<CapabilityPage<CommunicationMessage>>('testagram.messages.list', { conversation_id: conversationId, limit, ...(cursor ? { cursor } : {}) });
  },
  sendMessage(input: { conversationId: string; body: string; clientMessageId?: string; replyToMessageId?: string; sharedPostId?: string }) {
  ...
  editMessage(messageId: string, body: string) {
    return backendCapabilities.call<{ message_id: string; edited: boolean }>('testagram.messages.edit', { message_id: messageId, body });
  },
  deleteMessage(messageId: string) {
    return backendCapabilities.call<{ message_id: string; deleted: boolean }>('testagram.messages.delete', { message_id: messageId });
  },
  attachMessage(input: { messageId: string; mediaUrl: string; mediaType: string; mimeType?: string; byteSize: number; durationMs?: number; width?: number; height?: number }) {
    return backendCapabilities.call<{ attachment: MessageAttachment }>('testagram.messages.attach', {
  ...
  markMessageRead(messageId: string) {
    return backendCapabilities.call<{ message_id: string; read: boolean }>('testagram.messages.mark_read', { message_id: messageId });
  },
  reactToMessage(messageId: string, reaction: string, remove = false) {
    return backendCapabilities.call<{ message_id: string; reaction: string; removed: boolean }>('testagram.messages.react', { message_id: messageId, reaction, remove });
  },
  createCall(conversationId: string, kind: 'voice' | 'video' | 'screen' = 'video') {
    return backendCapabilities.call<CallSession>('testagram.calls.create', { conversation_id: conversationId, kind });
  },
  joinCall(callId: string) {
    return backendCapabilities.call<{ call_id: string; joined: boolean }>('testagram.calls.join', { call_id: callId });
  },
  endCall(callId: string) {
    return backendCapabilities.call<{ call_id: string; ended: boolean }>('testagram.calls.end', { call_id: callId });
  },
  ...
  /**
   * Private conversation channel. Postgres Changes remains the durable-message
   * path; Broadcast carries ephemeral typing/presence state on the same ACL'd topic.
   */
