...
// Novu remains downstream of canonical notifications rather than becoming a second inbox.
...
export const communicationService = {
  ...
  subscribeToConversation(conversationId: string, onMessage: (message: CommunicationMessage) => void) {
    const channel = supabase.channel(`testagram:conversation:${conversationId}`).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `conversation_id=eq.${conversationId}` }, payload => onMessage(payload.new as CommunicationMessage)).subscribe();
