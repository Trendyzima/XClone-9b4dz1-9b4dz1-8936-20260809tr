...
export const communicationCrypto = {
  ...
  async decryptMessage(conversationId: string, epoch: number, ciphertext: string, nonce: string, aad?: string) { const cached = await readConversationKey(conversationId, epoch); let key = cached?.key; if (!key) { const envelopes = (await this.listMyKeyEnvelopes(conversationId)).envelopes.filter(e => e.epoch === epoch); const envelope = envelopes[0]; if (!envelope) throw new Error(`Conversation key epoch ${epoch} unavailable`); key = await this.unwrapConversationKey(envelope); await storeConversationKey(conversationId, epoch, key); } const additionalData = new TextEncoder().encode(aad ?? `${conversationId}:${epoch}`); const plaintext = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: toArrayBuffer(decode(nonce)), additionalData: toArrayBuffer(additionalData) }, key, toArrayBuffer(decode(ciphertext))); return new TextDecoder().decode(plaintext); },
};
