...
const ensureDevice = async (): Promise<StoredDevice> => {
  ...
  await write(DEVICE_STORE, device);
  return device;
};
...
const deriveWrappingKey = async (privateKey: CryptoKey, publicKey: CryptoKey, salt: Uint8Array) => {
  ...
  return crypto.subtle.deriveKey({ name: 'HKDF', hash: 'SHA-256', salt: toArrayBuffer(salt), info: toArrayBuffer(INFO) }, material, { name: 'AES-GCM', length: 256 }, false, ['wrapKey', 'unwrapKey']);
};

const wrapKeyForDevice = async (conversationKey: CryptoKey, recipientPublicKey: string, senderCanonicalDeviceId: string) => {
  ...
  const wrapped = await crypto.subtle.wrapKey('raw', conversationKey, wrappingKey, { name: 'AES-GCM', iv: toArrayBuffer(nonce) });
  return { ciphertext: encode(wrapped), nonce: encode(toArrayBuffer(nonce)), salt: encode(toArrayBuffer(salt)), senderDeviceId: senderCanonicalDeviceId };
};
...
export const communicationCrypto = {
  ...
  async createConversationKey() { return crypto.subtle.generateKey({ name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']); },
  async wrapConversationKey(conversationKey: CryptoKey, recipientPublicKey: string, senderCanonicalDeviceId: string) { return wrapKeyForDevice(conversationKey, recipientPublicKey, senderCanonicalDeviceId); },
  async unwrapConversationKey(envelope: Envelope) {
    const device = await ensureDevice();
    const sender = await importPublicKey(envelope.sender_public_key);
    const wrappingKey = await deriveWrappingKey(device.privateKey, sender, decode(envelope.salt));
    return crypto.subtle.unwrapKey('raw', toArrayBuffer(decode(envelope.ciphertext)), wrappingKey, { name: 'AES-GCM', iv: toArrayBuffer(decode(envelope.nonce)) }, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
  },
  async syncConversationDevices(conversationId: string) {
    ...
    const envelopes = (await this.listMyKeyEnvelopes(conversationId)).envelopes;
    const latest = envelopes.sort((a, b) => b.epoch - a.epoch)[0];
    if (!latest) return false;
    const cached = await readConversationKey(conversationId, latest.epoch);
    if (!cached) return false;
    for (const recipient of devices) {
      const already = envelopes.some(e => e.epoch === latest.epoch && e.recipient_device_id === recipient.id);
      if (already) continue;
      const envelope = await this.wrapConversationKey(cached.key, recipient.identity_public_key, local.id);
      await this.putKeyEnvelope({ conversationId, epoch: latest.epoch, senderDeviceId: local.id, recipientDeviceId: recipient.id, ...envelope });
    }
    return true;
  },
  async ensureConversationKey(conversationId: string) {
    const local = await this.registerDevice();
    const envelopes = (await this.listMyKeyEnvelopes(conversationId)).envelopes;
    const existing = envelopes.sort((a, b) => b.epoch - a.epoch).find(x => x.recipient_device_id === local.id);
    if (existing) {
      const cached = await readConversationKey(conversationId, existing.epoch);
      const key = cached?.key ?? await this.unwrapConversationKey(existing);
      await storeConversationKey(conversationId, existing.epoch, key);
      await this.syncConversationDevices(conversationId);
    ...
    if (envelopes.length > 0) throw new Error('Conversation key exists but is not distributed to this device');
    const conversationKey = await this.createConversationKey();
    const epoch = 1;
    for (const recipient of devices) { const envelope = await this.wrapConversationKey(conversationKey, recipient.identity_public_key, local.id); await this.putKeyEnvelope({ conversationId, epoch, senderDeviceId: local.id, recipientDeviceId: recipient.id, ...envelope }); }
    await storeConversationKey(conversationId, epoch, conversationKey);
    return { epoch, key: conversationKey };
  },
  async encryptMessage(conversationId: string, plaintext: string) {
    ...
    const nonce = crypto.getRandomValues(new Uint8Array(12));
    const aad = new TextEncoder().encode(`${conversationId}:${epoch}`);
    const ciphertext = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: toArrayBuffer(nonce), additionalData: toArrayBuffer(aad) }, key, new TextEncoder().encode(plaintext));
    return { epoch, ciphertext: encode(ciphertext), nonce: encode(toArrayBuffer(nonce)), aad: `${conversationId}:${epoch}` };
  },
  async decryptMessage(conversationId: string, epoch: number, ciphertext: string, nonce: string, aad?: string) {
    const cached = await readConversationKey(conversationId, epoch);
    let key = cached?.key;
    if (!key) {
      const envelopes = (await this.listMyKeyEnvelopes(conversationId)).envelopes.filter(e => e.epoch === epoch);
      const envelope = envelopes[0];
      if (!envelope) throw new Error(`Conversation key epoch ${epoch} unavailable`);
      key = await this.unwrapConversationKey(envelope);
      await storeConversationKey(conversationId, epoch, key);
    }
    const additionalData = new TextEncoder().encode(aad ?? `${conversationId}:${epoch}`);
    const plaintext = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: toArrayBuffer(decode(nonce)), additionalData: toArrayBuffer(additionalData) }, key, toArrayBuffer(decode(ciphertext)));
