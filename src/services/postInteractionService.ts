...
export async function togglePostLike(postId: string): Promise<LikeState> {
  ...
  if (isRemoteStatus(postId)) {
    const data = await remoteAction('favorite', { post_id: postId });
    const delivered = data?.delivery?.status === 'delivered' || data?.delivery?.queue?.status === 'delivered';
    trackTestagramEvent(delivered ? TestagramEvent.POST_LIKED : TestagramEvent.POST_UNLIKED, { post_id: postId });
    return { is_liked: delivered, likes_count: Number(data?.remote?.like_count ?? data?.likes_count ?? 0) };
  }
...
export async function togglePostRepost(postId: string): Promise<RepostState> {
  if (!postId) throw new Error('Post id is required');
  if (isRemoteStatus(postId)) {
    const data = await remoteAction('boost', { post_id: postId });
    const delivered = data?.delivery?.status === 'delivered' || data?.delivery?.queue?.status === 'delivered';
    trackTestagramEvent(delivered ? TestagramEvent.POST_REPOSTED : TestagramEvent.POST_UNREPOSTED, { post_id: postId });
    return { is_reposted: delivered, reposts_count: Number(data?.remote?.announce_count ?? data?.reposts_count ?? 0) };
  }
