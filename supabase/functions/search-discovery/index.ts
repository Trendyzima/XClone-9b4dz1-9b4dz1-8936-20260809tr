...
const json = (body: unknown, status = 200, requestId = crypto.randomUUID()) => new Response(JSON.stringify(body), {
  status,
  headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Headers": "authorization, apikey, content-type, x-request-id", "Access-Control-Allow-Methods": "POST, OPTIONS", "X-Request-Id": requestId },
});
...
Deno.serve(async (req) => {
  ...
  return withServiceMetric("search", "discovery", async () => {
    try {
      ...
      const firstError = [usersRes, postsRes, hashtagsRes, communitiesRes].find((r: any) => r?.error)?.error;
      ...
      return json({ ok: false, data: null, error: { code: message === "INVALID_QUERY" ? "INVALID_QUERY" : "SEARCH_FAILED", message }, request_id: requestId }, message === "INVALID_QUERY" ? 400 : 500, requestId);
    }
  });
});
