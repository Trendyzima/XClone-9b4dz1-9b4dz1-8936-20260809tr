...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: existing, error: existingError } = await admin.from("testagram_advertisers").select("id,name,status").eq("owner_user_id", user.id).eq("status", "active").order("created_at", { ascending: true }).limit(1).maybeSingle();
    if (existingError) throw existingError;
    let advertiser = existing;
    if (!advertiser) {
    ...
    // Payment is a separate state from campaign lifecycle. A newly-created campaign is a draft until funded and published.
    const { data: campaign, error: campaignError } = await admin.from("testagram_ad_campaigns").insert({ advertiser_id: advertiser.id, name, objective: String(body.objective ?? "traffic"), status: "draft", payment_status: "unpaid", currency: "KES", lifetime_budget_micros: budgetMicros, daily_budget_micros: body.daily_budget_kes == null ? null : Math.round(Number(body.daily_budget_kes) * 1_000_000), bid_cpm_micros: bidCpmMicros, priority: Math.max(0, Math.min(1000, Number(body.priority ?? 50))), starts_at: startAt.toISOString(), ends_at: endAt.toISOString(), targeting }).select("id,advertiser_id,name,status,payment_status,currency,lifetime_budget_micros,starts_at,ends_at").single();
    ...
    return json({ ok: true, owner: "testagram", campaign, creative });
  } catch (error) {
