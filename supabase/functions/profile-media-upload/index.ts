...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (kind !== "avatar" && kind !== "cover") return json({ error: "kind must be avatar or cover" }, 400);
    if (!(file instanceof File)) return json({ error: "Image file is required" }, 400);
    if (!ALLOWED.has(file.type.toLowerCase())) return json({ error: "Unsupported profile image type" }, 415);
    const max = kind === "avatar" ? MAX_AVATAR : MAX_COVER;
    if (file.size <= 0 || file.size > max) return json({ error: `Profile ${kind} must be ${max / 1024 / 1024}MB or smaller` }, 413);

    const key = `profiles/${user.id}/${kind}.${extension(file.type.toLowerCase())}`;
    const bytes = new Uint8Array(await file.arrayBuffer());
    await r2.send(new PutObjectCommand({ Bucket: BUCKET, Key: key, Body: bytes, ContentType: file.type.toLowerCase(), ContentLength: file.size, Metadata: { ownerId: user.id, profileMedia: kind } }));
    ...
    if (error) return json({ error: "Image uploaded but profile update failed" }, 500);

    return json({ ok: true, kind, delivery_url: deliveryUrl, object_key: key, size_bytes: file.size, mime_type: file.type.toLowerCase(), etag: head.ETag ?? null });
  } catch (error) {
    console.error("profile-media-upload error", error);
    return json({ error: "Profile media operation failed" }, 500);
  }
});
