...
begin
 ...
 if kind in ('all','posts','media','latest','top') then select coalesce(jsonb_agg(to_jsonb(x)),'[]') into vp from (select p.id,p.author_id,p.content,p.created_at,p.updated_at,p.image_url,p.video_url,p.is_video,p.media_urls,p.likes_count,p.reposts_count,p.replies_count,p.quotes_count,p.views_count,p.visibility from posts p where p.deleted_at is null and p.visibility='public' and (q='' or lower(coalesce(p.content,'')) like '%'||lower(q)||'%') and (kind<>'media' or p.image_url is not null or p.video_url is not null or coalesce(array_length(p.media_urls,1),0)>0) order by case when kind='top' then coalesce(p.likes_count,0)+coalesce(p.reposts_count,0)+coalesce(p.replies_count,0) else 0 end desc,p.created_at desc limit n)x; end if;
