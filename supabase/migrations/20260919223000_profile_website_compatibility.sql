-- Keep profile website naming compatible with the settings UI and legacy backend.
alter table public.profiles add column if not exists website text;
update public.profiles set website=website_url where website is null and website_url is not null;

create or replace function public.profile_update(p_input jsonb default '{}'::jsonb)
returns jsonb language plpgsql security invoker set search_path=public
as $$
declare v_user_id uuid:=auth.uid(); v_profile public.profiles%rowtype; v_username text; v_website text;
begin
 ...
 if p_input ? 'username' then v_username:=lower(btrim(p_input->>'username')); if v_username='' or length(v_username)>30 or v_username !~ '^[a-z0-9_][a-z0-9_.-]{1,29}$' then raise exception using errcode='22023',message='Invalid username'; end if; end if;
 ...
 if p_input ? 'birth_date' and nullif(btrim(p_input->>'birth_date'),'') is not null then begin perform (p_input->>'birth_date')::date; exception when others then raise exception using errcode='22023',message='Invalid birth date'; end; end if;
 ...
 updated_at=now() where id=v_user_id returning * into v_profile;
