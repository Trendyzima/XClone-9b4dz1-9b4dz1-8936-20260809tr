...
create index if not exists content_recommendations_user_score_idx
  on public.content_recommendations (user_id, shown, score desc, created_at desc);
create index if not exists content_recommendations_post_idx
  on public.content_recommendations (recommended_post_id);
...
begin
  if p_user_id is null or p_user_id <> auth.uid() then
    raise exception 'recommendation user mismatch';
  end if;
  ...
  muted as (
    select um.muted_id as user_id from public.user_mutes um where um.muter_id = p_user_id
  ),
  blocked as (
    select ub.blocked_id as user_id from public.user_blocks ub where ub.blocker_id = p_user_id
  ),
  seen as (
  ...
  second_degree as (
    ...
    from public.follows f1 join public.follows f2 on f2.follower_id = f1.following_id
  ...
  ranked as (
    select c.*, (
      8.0 * c.is_following + least(18.0, c.interest_score * 4.0) + 3.0 * c.is_second_degree
      + 2.0 * ln(1 + greatest(c.likes_count, 0)) + 3.0 * ln(1 + greatest(c.reposts_count, 0))
      + 1.5 * ln(1 + greatest(c.replies_count, 0)) + 0.25 * ln(1 + greatest(c.views_count, 0))
      + case when c.is_video then 2.0 else 0 end + case when coalesce(c.media_count, 0) > 0 then 1.0 else 0 end
    ) * exp(-extract(epoch from (now() - c.created_at)) / 86400.0 / 1.5) as score
    from candidates c
  ),
  diversified as (
    select r.*, row_number() over (partition by r.user_id order by r.score desc, r.created_at desc) as author_rank
    from ranked r
  )
  ...
  select p_user_id, d.id, d.score,
    case when d.is_following = 1 then 'From someone you follow'
         when d.interest_score > 0 then 'Matches your interests'
         when d.is_second_degree = 1 then 'Popular with people in your network'
         else 'Trending now' end,
    d.source, false
  from diversified d
